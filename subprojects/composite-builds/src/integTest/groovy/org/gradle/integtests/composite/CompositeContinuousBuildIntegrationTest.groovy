...
class CompositeContinuousBuildIntegrationTest extends Java7RequiringContinuousIntegrationTest {
    ...
    def "can use continuous build with composite builds and Java libraries"() {
    ...
    def "can use continuous build with composite builds for plugin development"() {
    ...
    def "can use continuous build with composite builds in a multi-project build"() {
        def includedLibrary = singleProjectBuild("library") {
            buildFile << """
                apply plugin: 'java'
            """

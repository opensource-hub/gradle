...
public class DefaultBuildCacheConfiguration implements BuildCacheConfigurationInternal {
    ...
    private static <T extends BuildCache> T createBuildCacheConfiguration(Instantiator instantiator, Class<T> type, Set<BuildCacheServiceRegistration> registrations) {
        Class<? extends BuildCacheServiceFactory<T>> factoryType = findBuildCacheServiceFactoryType(type, registrations);
        if (factoryType == null) {
            throw new GradleException("Build cache type '" + type.getName() + "' has not been registered.");
        }
    ...
    public <T extends BuildCache> Class<? extends BuildCacheServiceFactory<T>> getBuildCacheServiceFactoryType(Class<T> configurationType) {
        Class<? extends BuildCacheServiceFactory<T>> buildCacheServiceFactoryType = findBuildCacheServiceFactoryType(configurationType, registrations);
        if (buildCacheServiceFactoryType == null) {
        ...
        return buildCacheServiceFactoryType;
    }

    @Nullable
    private static <T extends BuildCache> Class<? extends BuildCacheServiceFactory<T>> findBuildCacheServiceFactoryType(Class<T> configurationType, Set<BuildCacheServiceRegistration> registrations) {
